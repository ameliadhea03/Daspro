...
public class CinemaWithScanner03_3{
        ...
        while (true) {
                ...
                switch (choice) {
                    ...
                    do {
                        ...
                        validSeat = isValidSeat(selectedRow, selectedColumn, audience);
                        if (!validSeat){
                            System.out.println("Seat is already occupied by other audiences");
                        }
                        } while (!validSeat);

                        audience[selectedRow -1][selectedColumn -1] = name;
                        break;
    ...
    private static boolean isValidSeat(int row, int column, String[][] audience) {
        if (row >= 1 && row <= 4 && column >= 1 && column <= 2 && audience[row - 1][column - 1] == null){
            return true;
        }
        return false;
